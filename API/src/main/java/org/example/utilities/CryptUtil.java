...
public class CryptUtil {
    ...
    @Value("${crypt-util.secret-key}")
    private String KEY_ENV_VAR;

    @Value("${crypt-util.algorithm}")
    private String ALGORITHM;

    public String encrypt(String valueToEnc) throws RuntimeException {
        Key key = generateKey();
        try {
    ...
    public String decrypt(String encryptedValue) throws RuntimeException {
        try {
            Cipher c = Cipher.getInstance(ALGORITHM);
